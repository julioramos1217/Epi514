...
endyear <- as.Date("10-29-2016", format = "%M-%d-%Y")
...
tester1 <- subset(LLCP2016, X_HISPANC == 1)
tester2 <- subset(LLCP2018, X_HISPANC == 1)
tester3 <- subset(LLCP2020, X_HISPANC == 1)
...
testercomplete %>% select(diabebin, cvdbin, activbin, year, bmihigh, X_AGE_G, sex) %>%
 tbl_summary(by = "year",missing = "ifany",
 digits = list(all_categorical() ~ c(0,1)), 
 label = list(diabebin ~ "Diabetes",
              cvdbin ~ "CVD", 
              ...
              sex ~ "Sex")) %>%
 modify_header(label = "**Variable**") %>%
  modify_caption("**Table 1. National Characteristics Among Latin Population for Physical Activity, Diabetes, Cardiovascular Disease, Body Mass Index, Age Group, Sex, Based on BRFSS Interview Years 2016-2021**") %>%
  bold_labels() 
